...
export function SiteShell({ active, children, className = "" }: SiteShellProps) {
  ...
  const navItems = [{ label: "Home", route: "/", section: "#home-hero", key: "home" }, { label: "Work", route: "/work", section: "#home-work", key: "work" }, { label: "About", route: "/about", section: "#home-about", key: "about" }, { label: "Contact", route: "/contact", section: "#home-contact", key: "contact" }] as const;
  return <main ref={pageRef} className={`reference-page ${className}`}><header className="reference-header"><Link className="reference-logo" to="/" aria-label="SO. home">SO.</Link><nav className="reference-nav" aria-label="Primary navigation">{navItems.map((item) => isHomepage ? <a className={visibleSection === item.key ? "is-active" : ""} href={item.section} key={item.key}>{item.label}</a> : <Link className={active === item.key ? "is-active" : ""} to={item.route} key={item.key}>{item.label}</Link>)}</nav></header>{children}<aside className="social-rail" aria-label="Social links"><a href="https://github.com/shrestha-shubham" target="_blank" rel="noreferrer" aria-label="GitHub"><Github size={18} /></a><a href="https://www.linkedin.com/in/shubham-shrestha-01a4183ba/" target="_blank" rel="noreferrer" aria-label="LinkedIn"><Linkedin size={18} /></a><a href="mailto:shresthashubham618@gmail.com" aria-label="Email"><Mail size={19} /></a></aside><footer className="reference-footer"><small>© SO. ALL RIGHTS RESERVED.</small></footer></main>;
}
